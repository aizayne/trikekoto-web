...
/// happy path: money counted from rides nobody took, a completion rate
/// dragged down by trips still in progress, or a driver credited for a ride
/// they never got.
...
void main() {
  ...
  group('money', () {
    test('only completed rides contribute fares', () {
      final a = _analyse([
        _ride(status: RideStatus.completed, fare: 50),
        // Both of these carry the estimate they were quoted at. Counting
        // them would bill the operator for trips nobody took.
        _ride(status: RideStatus.cancelled, fare: 90),
        _ride(status: RideStatus.expired, fare: 90),
        _ride(status: RideStatus.inTransit, fare: 90),
      ]);

      expect(a.fares, 50);
      expect(a.averageFare, 50);
    });

    test('a completed ride with no fare recorded does not skew the average',
        () {
      final a = _analyse([
        _ride(status: RideStatus.completed, fare: 60),
        _ride(status: RideStatus.completed),
      ]);

      // Averaged over completed rides, so a missing fare pulls the average
      // down rather than being silently dropped from the denominator. That
      // is the honest reading: the ride happened and earned nothing recorded.
      expect(a.fares, 60);
      expect(a.averageFare, 30);
    });

    test('average distance covers completed rides only', () {
      final a = _analyse([
        _ride(status: RideStatus.completed, distanceKm: 3),
        _ride(status: RideStatus.completed, distanceKm: 5),
        _ride(status: RideStatus.cancelled, distanceKm: 100),
      ]);
      expect(a.averageDistanceKm, 4);
    });
  });
  ...
  group('per driver', () {
    test('attributes rides, fares and ratings to the assigned driver', () {
      final a = _analyse([
        _ride(
        ...
        _ride(
            ...
            distanceKm: 3,
            rating: 3),
        _ride(status: RideStatus.cancelled, driver: 'ana@x.ph', fare: 99),
      ]);
      ...
      expect(ana.fares, 100);
      expect(ana.distanceKm, 5);
    ...
    test('an expired offer belongs to nobody', () {
      final a = _analyse([
        _ride(status: RideStatus.expired),
        _ride(status: RideStatus.completed, driver: 'ana@x.ph', fare: 10),
      ]);
    ...
    test('ranks by completed rides, not by fares', () {
      final a = _analyse([
        // One long trip.
        _ride(status: RideStatus.completed, driver: 'long@x.ph', fare: 500),
        // Six short ones — the driver a dispatcher actually wants to see top.
        for (var i = 0; i < 6; i++)
          _ride(
              status: RideStatus.completed,
              driver: 'busy@x.ph',
      ...
      expect(a.drivers.last.email, 'long@x.ph');
    });

    test('falls back to fares when two drivers finished the same count', () {
      final a = _analyse([
        _ride(status: RideStatus.completed, driver: 'a@x.ph', fare: 30),
        _ride(status: RideStatus.completed, driver: 'b@x.ph', fare: 80),
      ]);
  ...
  group('bad data', () {
    test('a ride with no createdAt is skipped rather than crashing', () {
      ...
      final a = _analyse([legacy, _ride(status: RideStatus.completed, fare: 10)]);
      expect(a.completed, 1);
      expect(a.fares, 10);
    });
