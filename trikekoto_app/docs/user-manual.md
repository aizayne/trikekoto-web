...
*Version 1.0.6 (7) · September 2026*
...
> **Note:** This manual describes version 1.0.6 (7). The version is printed at the bottom of the landing screen, the driver screen, the commuter Profile screen and the ID verification screen.
...
2. Type the number printed on the card in **Numero ng ID** (ID number), without dashes or spaces. The box accepts only as many characters as that ID uses, and says how many below it (see the table).
...
4. Under **Uri ng biyahe** (Type of trip), choose **Regular — 1 pasahero** (Regular — 1 passenger) or **Special — buong tricycle** (Special — whole tricycle) (see the table below). Regular is chosen unless you change it.
5. Check your name and mobile number, so the driver can find and call you.
6. Tap **Maghanap ng driver** (Find a driver).

| Type of trip | What it means |
|---|---|
| **Regular — 1 pasahero** (Regular — 1 passenger) | You pay the fare for one passenger. The driver may take other passengers going the same way. |
| **Special — buong tricycle** (Special — whole tricycle) | You agree to pay the fare for five passengers, and the tricycle is yours alone. The driver sees this before accepting. |
...
### 4.10 Your ride history

Tap **Kasaysayan ng biyahe** (Ride history) below the **Maghanap ng driver** (Find a driver) button. Your rides are listed newest first, 30 at a time; tap **Ipakita pa** (Show more) for older ones.

![Figure 5. Ride history](manual/fig-8-ride-history.png)

*Figure 5. Ride history*

Each ride shows the date, the route, how it ended — **Natapos** (Completed), **Kinansela ng pasahero** (Cancelled by commuter), **Kinansela ng driver** (Cancelled by driver) or **Walang nakitang driver** (No driver found) — whether it was Regular or Special, the driver’s name and plate, and the stars you gave.
...
![Figure 6. The driver screen with the Online switch](manual/fig-5-driver-online.png)

*Figure 6. The driver screen with the Online switch*
...
Each offer is labelled **Regular — 1 pasahero** (Regular — 1 passenger) or **Special — bayad para sa 5 pasahero** (Special trip — fare for 5 passengers). On a special trip the commuter pays the fare for five passengers and has the tricycle alone, so do not take other passengers.
...
### 5.7 Your ride history

Tap the clock icon at the top of the driver screen to open **Kasaysayan ng biyahe** (Ride history). It lists your completed and cancelled rides, newest first, with the route, the trip type, the passenger’s name and their rating. Passengers’ phone numbers are not shown once a ride is over.
...
The administrator dashboard has shortcuts to **Feedback**, **Dispatch**, **ID review** and **Kasaysayan ng biyahe** (Ride history), a ride summary, and the list of drivers.

![Figure 7. The administrator dashboard](manual/fig-6-admin-dashboard.png)

*Figure 7. The administrator dashboard*
...
![Figure 8. Reviewing a submitted ID](manual/fig-7-id-review.png)

*Figure 8. Reviewing a submitted ID*
...
Open **Kasaysayan ng biyahe** (Ride history) from the dashboard. Every ride in the chapter is listed, newest first, with the commuter’s name and number, the driver, the trip type, the outcome and the rating. Use **Lahat** (All), **Natapos** (Completed), **Kinansela** (Cancelled) or **Walang nakitang driver** (No driver found) to show one outcome only.
...
| **Kailangan ng 16 numero** (Must be exactly 16 digits) (or another count) | The number is too short or too long for the ID type chosen. Check the ID type, then type every digit on the card, without dashes. |
